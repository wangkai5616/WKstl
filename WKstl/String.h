...
namespace WKstl {
	...
	private:
		...
		typedef WKstl::allocator<char> dataAllocator;
	public:
		...
		void swap(string& str) {
			WKstl::swap(start_, str.start_);
			WKstl::swap(finish_, str.finish_);
			WKstl::swap(endOfStorage_, str.endOfStorage_);
		}
		size_t copy(char* s, size_t len, size_t pos = 0) const {
			auto ptr = WKstl::uninitialized_copy(begin() + pos, begin() + pos + len, s);
			return (size_t)(ptr - s);
		}
	...
	string::iterator string::insert_aux_copy(iterator p, InputIterator first, InputIterator last) {
		...
		iterator newFinish = WKstl::uninitialized_copy(start_, p, newStart);
		newFinish = WKstl::uninitialized_copy(first, last, newFinish);
		auto res = newFinish;
		newFinish = WKstl::uninitialized_copy(p, finish_, newFinish);
	...
	string::iterator string::insert(iterator p, InputIterator first, InputIterator last) {
		...
		if (lengthOfInsert <= lengthOfLeft) {
			...
			WKstl::uninitialized_copy(first, last, p);
