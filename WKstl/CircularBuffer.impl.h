#ifndef _CIRCULARBUFFER_IMPL_H_
#define _CIRCULARBUFFER_IMPL_H_
namespace WKstl {
	namespace Detail {
		template<class T,size_t N,class Alloc>
		int cb_iter<T, N, Alloc>::preIndex(int index) {
			--index;
		...
		cb_iter<T, N, Alloc> operator +(const cb_iter<T, N, Alloc>& cit, std::ptrdiff_t i) {
			int real_i = i % (std::ptrdiff_t)N;
			if (real_i < 0)
				real_i += N;
			cb_iter<T, N, Alloc> res = cit;
			res.setIndex_(res.index_ + real_i);
			res.setPtr_(res.ptr_ + res.index_);//此时的index_是更新后的
			return res;
		}
		template<class T, size_t N, class Alloc>
		cb_iter<T, N, Alloc> operator -(const cb_iter<T, N, Alloc>& cit, std::ptrdiff_t i) {
	...
	circular_buffer<T, N, Alloc>::~circular_buffer() {
		clear();
		dataAllocator::deallocator(start_, size_);
	}
	template<class T, size_t N, class Alloc>
	...
	void circular_buffer<T, N, Alloc>::allocateAndFillN(const int& n, const value_type& val) {
		...
		indexOfHead = 0;
		if (N <= n) {
			finish_ = WKstl::uninitialized_fill_n(start_, N, val);
			indexOfTail = N - 1;
			size_ = N;
		}
		else {
			finish_ = WKstl::uninitialized_fill_n(start_, n, val);
			finish_ = WKstl::uninitialized_fill_n(finish_, N-n, value_type());
			indexOfTail = n - 1;
			size_ = n;
		}
	}
	...
	void circular_buffer<T, N, Alloc>::allocateAndCopy(InputIterator first, InputIterator last) {
		...
		indexOfHead = 0;
		if (N <= n) {
			finish_ = WKstl::uninitialized_copy(first, first+N, start_);
			indexOfTail = N - 1;
			size_ = N;
		}
		else {
	...
	void circular_buffer<T, N, Alloc>::clone(const circular_buffer& cb) {
		...
		indexOfHead = cb.indexOfhead;
		indexOfTail = cb.indexOfTail;
		WKstl::uninitialized_copy(cb.start_, cb.finish_, start_);
	}
}
#endif