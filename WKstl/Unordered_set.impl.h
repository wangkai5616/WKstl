...
namespace WKstl {
	...
	bool Unordered_set<Key, Hash, KeyEqual, Allocator>::has_key(const key_type& key) {
		...
		return WKstl::find_if(list.begin(), list.end(), pred) != list.end();
	}
	...
	//总共提供了28条list，你看看你有多少元素决定用哪条list
	//参数是数组个数,也就是桶的个数
	template<class Key, class Hash, class KeyEqual, class Allocator>
	typename Unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
	...
	//bucket_count到底是桶的个数还是元素
	//max_load_factor代表着什么
	template<class Key, class Hash, class KeyEqual, class Allocator>
	...
	template<class Key, class Hash, class KeyEqual, class Allocator>
	WKstl::pair<typename Unordered_set<Key, Hash, KeyEqual, Allocator>::iterator, bool>
		...
		if (!has_key(val)) {
			...
			++size_;
			return WKstl::pair<iterator, bool>(iterator(index, buckets_[index].begin(), this), true);
		}
		return WKstl::pair<iterator, bool>(end(), false);
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	template<class InputIterator>
	...
	/*
	设置要对容器进行计数和重新映射的桶数，即考虑到桶的总数已更改，
	将这些元素放入适当的桶中。 如果新桶的数量使得加载因子超过最大负
	载因子（count <size（）/ max_load_factor（）），
	那么新的桶数至少为size（）/ max_load_factor（）。
	*/
	//重新设置桶的个数
	template<class Key, class Hash, class KeyEqual, class Allocator>
	void Unordered_set<Key, Hash, KeyEqual, Allocator>::rehash(size_type n) {
		...
		WKstl::swap(*this, temp);
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	void swap(Unordered_set<Key, Hash, KeyEqual, Allocator>& lhs,
		Unordered_set<Key, Hash, KeyEqual, Allocator>& rhs) {
		WKstl::swap(lhs.buckets_, rhs.buckets_);
		WKstl::swap(lhs.size_, rhs.size_);
		WKstl::swap(lhs.max_load_factor_, rhs.max_load_factor_);
	}
